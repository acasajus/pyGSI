...
int gsiVerifyCallback( int ok, X509_STORE_CTX *ctx )
{
   ...
   {
      if (gsiCheckKnownCriticalExt(X509_STORE_CTX_get_current_cert(ctx))
                                                              == GSI_RET_OK)
      {
         logMsg( 0, "gsiCheckKnownCriticalExt() accepts previously:  Unhandled Critical Extension (GSI Proxy?)" );
...
/// introduced in 0.9.7.
{
   ...
   for (i = 0; i < X509_get_ext_count(cert); ++i)
   {
                           ...
                           !X509_supported_extension(ex))
      {
         ...
         if (strcmp(s, GSI_PROXYCERTINFO_OID) != 0) return GSI_RET_FAILED;
      }
   }
...
unsigned long gsiVerifyProxyChain( STACK_OF(X509) *certstack )
{
   ...
   for( i = depth-2; i>=0; i-- )
   {
      ...
      if (now < gsiAsn1TimeToTimeT(ASN1_STRING_data(X509_get_notBefore(cert)),0))
      {
         logMsg( 2, "Proxy certificate is not yet valid." );
         foundError |= X509_V_ERR_CERT_NOT_YET_VALID;
      }

      if (now > gsiAsn1TimeToTimeT(ASN1_STRING_data(X509_get_notAfter(cert)),0))
      {
