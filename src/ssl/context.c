...
static char *CVSid =
    "@(#) $Id: context.c,v 1.8 2008/07/08 10:54:55 acasajus Exp $";
...
global_verify_callback( int ok, X509_STORE_CTX * x509_ctx )
{
    ...
    if ( conn->context->verify_callback != Py_None )
    {
        ...
        argv = Py_BuildValue( "(OOiii)", ( PyObject * ) conn,
                              ( PyObject * ) cert, errnum, errdepth, ok );
        Py_DECREF( cert );
 ...
 */
static int
gsi_wrapper_global_verify_callback( int ok, X509_STORE_CTX * x509_ctx )
{
...
ssl_Context_set_verify( ssl_ContextObj * self, PyObject * args )
{
    ...
    if ( !PyArg_ParseTuple
         ( args, "iO|i:set_verify", &mode, &callback, &gsiEnable ) )
        return NULL;
    ...
    if ( gsiEnable )
    {
        ...
        SSL_CTX_set_verify( self->ctx, mode,
                            gsi_wrapper_global_verify_callback );
    }
    else
